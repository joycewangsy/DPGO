...
from source.points_to_surf_model import normalizePointsToSurfModelk4_add_decodertest as PointsToSurfModel
#from source.points_to_surf_model import PointsToSurfModel
from source import normalize_data_loader as data_loader
...
def parse_arguments(args=None):
    ...
    parser.add_argument('--name', type=str, default='base_k4_normalize_adddecoder_lossnew_0.3'),
    parser.add_argument('--desc', type=str, default='My training run for single-scale normal estimation.',
                        help='description')
    parser.add_argument('--indir', type=str, default='/home/mark/下载/pcp_datasets',
                        help='input folder (meshes)')
    parser.add_argument('--outdir', type=str, default='/home/mark/Experiment/wsy/our_normals/models',
                        help='output folder (trained models)')
    parser.add_argument('--logdir', type=str, default='/home/mark/Experiment/wsy/our_normals/logs',
                        help='training log folder')
    parser.add_argument('--trainset', type=str, default='trainingset_whitenoise.txt',
                        help='training set file name')
    parser.add_argument('--testset', type=str, default='validationset_whitenoise.txt',
    ...
    parser.add_argument('--cache_capacity', type=int, default=30,
    ...
    parser.add_argument('--shared_transformer', type=int, default=1,
                        help='use a single shared QSTN that takes both the local and global point sets as input')
    parser.add_argument('--training_order', type=str, default='random_shape_consecutive',
    ...
    parser.add_argument('--normal_loss', type=str, default='ms_euclidean_exploss_new',
                        help='Normal loss type:\n'
                        'ms_euclidean: mean square euclidean distance\n'
                        'ms_oneminuscos: mean square 1-cos(angle error)'
                        'all:mean square 1-cos(angle error) and mean square euclidean distance')
...
def do_logging(writer, log_prefix, epoch, opt, loss,
    ...
    if batchind % opt.debug_interval == 0:
        state_string = \
            '[{name} {epoch}: {batch}/{n_batches}] {prefix} loss: {loss:+.2f}'.format(
...
def points_to_surf_train(opt):
    ...
    for o in opt.outputs:
        if o == 'imp_surf':
            ...
            output_target_ind.append(target_features.index(o))
        #wsy: add normals in outputs;output 3-dim
        elif o == 'normals':
            if o not in target_features:
                target_features.append(o)
            output_names.append(o)
    ...
    # create model
    use_query_point = any([f in opt.outputs for f in ['normals']])
    ...
    start_epoch = 0
    if opt.refine != '':
        print('Refining weights from {opt.refine}')
        ...
        try:
            ...
            print('Warning: {opt.refine} has no epoch in the name. The Tensorboard log will continue at '
                  'epoch 0 and might be messed up!')
    ...
    for epoch in range(start_epoch, opt.nepoch, 1):
        ...
        for train_batchind, batch_data_train in train_enum:
            ...
            # zero gradients
            optimizer.zero_grad()
            #pred_train,trans,_ = p2s_model(batch_data_train)
            pred_train,normal,trans,_ = p2s_model(batch_data_train)
            #pred_train=sdf_nn.post_process_sign(pred_train)
            #sign=pred_train
            #pred_train=normal*pred_train
            ...
            while test_fraction_done <= train_fraction_done and test_batchind + 1 < test_num_batch:
                ...
                # forward pass
                with torch.no_grad():
                    pred_test,normal_test,trans,_ = p2s_model(batch_data_test)
                #pred_test=sdf_nn.post_process_sign(pred_test)
                #sign_test=pred_test
                #pred_test=normal*pred_test
...
def compute_loss(pred,local,s,neighbor,weight,batchsize,batch_data, outputs, output_loss_weights, fixed_radius,normal_loss,use_point_stn,shared_transformation,trans):
    ...
    if 'normals' in outputs:
        ...
        if use_point_stn and shared_transformation:
            o_pred = torch.bmm(o_pred.unsqueeze(1),trans.transpose(2,1)).squeeze(dim=1)
            local= torch.bmm(local.unsqueeze(1),trans.transpose(2,1)).squeeze(dim=1)
            #neighbor=torch.bmm(neighbor.view(-1, 1, 3),
            #                           trans.transpose(2, 1).repeat(1, 300, 1, 1).view(-1, 3, 3)).view(batchsize, 300, 3)

        if normal_loss == 'ms_euclidean':
            n_loss += (o_pred - o_target).pow(2).sum(1).mean()
        elif normal_loss == 'ms_oneminuscos':
            n_loss += (1 - utils.cos_angle(o_pred, o_target)).pow(2).mean()
        elif normal_loss =='all':
            n_loss += (0.5*((o_pred - o_target).pow(2).sum(1).mean()) + 0.5*(
                (1 - utils.cos_angle(o_pred, o_target)).pow(2).mean()))
        elif normal_loss == 'add_abs':
            n_loss += (1 - utils.cos_angle(o_pred, o_target)).pow(2).mean()\
                      +torch.min((o_pred-o_target).pow(2).sum(1), (o_pred+o_target).pow(2).sum(1)).mean()
        elif normal_loss == 'ms_euclidean_add_loss':
            o_pred_direction_points = batch_data['imp_surf_query_point_ms'] + 0.1*o_pred
            o_target_direction_points = batch_data['imp_surf_query_point_ms'] + 0.1 * o_target
            normal_loss = torch.min((o_pred - o_target).pow(2).sum(1),
                                    (o_pred + o_target).pow(2).sum(1)).mean()
            loss_n1=torch.nn.L1Loss()
            point_loss = 10*loss_n1(o_pred_direction_points,o_target_direction_points)
            print(point_loss.item())
            n_loss += normal_loss + point_loss
        elif normal_loss == 'ms_euclidean_exploss':
            o_pred_direction_points = batch_data['imp_surf_query_point_ms'] + 0.1*o_pred
            o_target_direction_points = batch_data['imp_surf_query_point_ms'] + 0.1 * o_target
            normal_loss = torch.min((o_pred - o_target).pow(2).sum(1),
                                    (o_pred + o_target).pow(2).sum(1)).mean()
            loss_n1=torch.nn.L1Loss()
            point_loss = 10*loss_n1(o_pred_direction_points,o_target_direction_points)
            #consistency_loss = 0.25 * torch.mean(weight *
            #                                      torch.norm(torch.cross(neighbor.view(-1, 3),
            #                                                             neighbor_target.view(-1, 3), dim=-1).view(batchsize, -1, 3), p=2, dim=2))
            local_loss = torch.min((local - o_target).pow(2).sum(1),
                                    (local + o_target).pow(2).sum(1)).mean()
            #local_loss+=consistency_loss
            #point_loss=10*(o_pred_direction_points - o_target_direction_points).pow(2).sum(1).mean()
            print(point_loss.item())
            print(local_loss.item())
            n_loss += torch.exp(normal_loss + point_loss)-1+local_loss

        elif normal_loss == 'ms_euclidean_exploss_2':
            o_pred_direction_points = batch_data['imp_surf_query_point_ms'] + 0.1*o_pred
            o_target_direction_points = batch_data['imp_surf_query_point_ms'] + 0.1 * o_target
            normal_loss = torch.exp(torch.min((o_pred - o_target).pow(2).sum(1),
                                    (o_pred + o_target).pow(2).sum(1)).mean())-1
            loss_n1=torch.nn.L1Loss()
            #point_loss = torch.exp(loss_n1(o_pred_direction_points,o_target_direction_points))
            #point=(o_pred_direction_points - o_target_direction_points).pow(2).sum(1).mean()
            point=10*loss_n1(o_pred_direction_points,o_target_direction_points)
            point_loss = torch.exp(point)-1
            #point_loss=10*(o_pred_direction_points - o_target_direction_points).pow(2).sum(1).mean()
            print(point_loss.item())
            n_loss += normal_loss + point_loss

        elif normal_loss == 'ms_euclidean_exploss_3':
            o_pred_direction_points = batch_data['imp_surf_query_point_ms'] + 0.1*o_pred
            o_target_direction_points = batch_data['imp_surf_query_point_ms'] + 0.1 * o_target
            normal_loss = torch.min((o_pred - o_target).pow(2).sum(1),
                                    (o_pred + o_target).pow(2).sum(1)).mean()
            point_loss=50*(o_pred_direction_points - o_target_direction_points).pow(2).sum(1).mean()
            print(point_loss.item())
            n_loss += torch.exp(normal_loss + point_loss)-1
        elif normal_loss == 'ms_euclidean_exploss_modelnet':
            o_pred_direction_points = batch_data['imp_surf_query_point_ms'] + 0.2*o_pred
            o_target_direction_points = batch_data['imp_surf_query_point_ms'] + 0.2 * o_target
            normal_loss = torch.min((o_pred - o_target).pow(2).sum(1),
                                    (o_pred + o_target).pow(2).sum(1)).mean()
            loss_n1=torch.nn.L1Loss()
            point_loss = 10*loss_n1(o_pred_direction_points,o_target_direction_points)
            ...
            print(local_loss.item())
            n_loss += torch.exp(normal_loss + point_loss)-1+local_loss
        elif normal_loss == 'ms_euclidean_exploss_new':
            o_pred_direction_points = batch_data['imp_surf_query_point_ms'] + 0.1*o_pred
            o_target_direction_points = batch_data['imp_surf_query_point_ms'] + 0.1 * o_target
            normal_loss = torch.min((o_pred - o_target).pow(2).sum(1),
                                    (o_pred + o_target).pow(2).sum(1)).mean()
            loss_n1=torch.nn.L1Loss()
            point_loss = 0.3*loss_n1(o_pred,o_target)
            #consistency_loss = 0.25 * torch.mean(weight *
            #                                      torch.norm(torch.cross(neighbor.view(-1, 3),
            #                                                             neighbor_target.view(-1, 3), dim=-1).view(batchsize, -1, 3), p=2, dim=2))
            local_loss = torch.min((local - o_target).pow(2).sum(1),
        ...
        loss.append(n_loss)
    if 'imp_surf_magnitude' not in outputs and 'imp_surf_sign' in outputs:
        o_pred = pred[:, 0].squeeze()
        o_target = batch_data['imp_surf_dist_sign_ms'].squeeze()
        loss.append(sdf_nn.calc_loss_sign(pred=o_pred, target=o_target) *
                    output_loss_weights['imp_surf_sign'])
...
def calc_metrics(outputs, pred, gt_data):
    ...
    def compute_cosine(pred,gt):
        o_target = gt['normals']
        right=0.0
        angle_diff=torch.sqrt(torch.mean(torch.acos(utils.cos_angle(pred, o_target))))
        angle = angle_diff / 3.14 * 180
        angle = angle < 90
        right += float(sum(angle))
        acc = float(right / pred.size(0))
        return acc
    if 'imp_surf_magnitude' in outputs and 'imp_surf_sign' in outputs:
